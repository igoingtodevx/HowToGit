  ...
  return (
    <section className="panel terminal-panel" aria-labelledby="terminal-title">
      ...
      <div className="terminal-output" ref={outputRef} aria-live="polite">
        ...
        {state.terminal.map((entry) => <div className="terminal-entry" key={entry.id}><div className="terminal-command"><b>❯</b> {entry.input}</div>{entry.lines.map((item, index) => <pre className={`tone-${item.tone}`} key={index}>{item.text}</pre>)}{entry.explanationKey && state.mode === 'noob' && <p className="noob-explanation">↳ {t(entry.explanationKey)}</p>}</div>)}
      </div>
      ...
      <div className="quick-commands" aria-label={t('ui.quickCommands')}>{quickCommands.map((command) => <button type="button" key={command} onClick={() => onCommandValue(command)}>{command}</button>)}</div>
    </section>
  );
}
